...
void GCS_MAVLINK::send_JAI(){
    const AP_AHRS &ahrs = AP::ahrs();
    Vector3f gyro_ahrs;
    Vector3f euler;
    Vector3f acceleration;
    ahrs.get_secondary_attitude(euler);
    gyro_ahrs=ahrs.get_gyro();

    acceleration=ahrs.get_accel();

    Vector3f state_1=get_state_JAI();
    Vector3f state_2=get_state_JAI_2();
    Vector3f state_3=get_state_JAI_3();
    if(state_3.y>1.0){
        state_3.y=1.0;
    }else if(state_3.y<-1.0){
        state_3.y=-1.0;
    }
    mavlink_msg_jai_raw_send(chan,
                             AP_HAL::millis(),     // time since system boot
                             euler.y,
                             euler.x,
                             euler.z,
...
ap_message GCS_MAVLINK::mavlink_id_to_ap_message_id(const uint32_t mavlink_id) const
{
    ...
    static const struct {
        ...
        { MAVLINK_MSG_ID_JAI_RAW,               MSG_JAI_RAW},
#if HAL_WITH_MCU_MONITORING
