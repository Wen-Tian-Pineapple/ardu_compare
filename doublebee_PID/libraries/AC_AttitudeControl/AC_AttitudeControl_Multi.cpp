...
void AC_AttitudeControl_Multi::rate_controller_run()
{
    ...
    //set uncoupled mode
    _desired_pitch_angle_rad=radians(desired_roll_deg_gw);
    float time_gap=(AP_HAL::millis()-_time)/1000000.0;
    if(time_gap>0.025){
        time_gap=0.0025;
    }
    // _desired_pitch_angle_rate_dot=(_ang_vel_body_gw.x-_desired_pitch_angle_rate_rad)/time_gap;
    _desired_pitch_angle_rate_dot=(_ang_vel_body_gw.x-_desired_pitch_angle_rate_rad)/time_gap;
    _desired_pitch_angle_rate_rad=_ang_vel_body_gw.x;

    _time=AP_HAL::millis();

    true_error=calculate_true_error(gyro_latest.x,_ang_vel_body_gw.x,roll_now);

    _beta_command=get_BRw_yaw_pid().update_all(_ang_vel_body_gw.x, gyro_latest.x, _dt, _motors.limit.roll) + _actuator_sysid.x;
    _wheel_command_l=_motors.get_tilt_wheel_left_gw();
    _wheel_command_r=_motors.get_tilt_wheel_right_gw();
    _wheel_rate_l=rate_left;
    _wheel_rate_r=rate_right;

    _motors.set_yaw_gw(_beta_command);
    _motors.set_yaw_ff_gw(get_BRw_yaw_pid().get_ff());
    if(jai_thrust_state==0||jai_thrust_state==1){
        _thr_out_command=get_BRw_roll_pid().update_all(true_error, 0, _dt, _motors.limit.roll) + _actuator_sysid.x;
    }else{
        _thr_out_command=jai_thrust_out;
    }
    // _thr_out_command=get_BRw_roll_pid().update_all(true_error, 0, _dt, _motors.limit.roll) + _actuator_sysid.x;
    _motors.set_roll_gw(_thr_out_command);
    _motors.set_roll_ff_gw(get_BRw_roll_pid().get_ff());

    _motors.set_pitch_gw(get_BRw_pitch_pid().update_all(_ang_vel_body_gw.y, gyro_latest.y, _dt, _motors.limit.pitch) + _actuator_sysid.y);
    _motors.set_pitch_ff_gw(get_BRe_pitch_pid().get_ff());
