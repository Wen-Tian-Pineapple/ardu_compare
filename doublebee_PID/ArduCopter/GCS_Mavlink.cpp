...
Vector3f GCS_MAVLINK_Copter::get_state_JAI(){
    // return Vector3f(copter.attitude_control->_wheel_rate_r,copter.attitude_control->_wheel_rate_l,copter.motors->yaw_gw);
    return Vector3f(copter.attitude_control->_wheel_rate_r,copter.attitude_control->_wheel_rate_l,copter.attitude_control->jai_thrust_state);

}


Vector3f GCS_MAVLINK_Copter::get_state_JAI_2(){
    return Vector3f(copter.motors->pitch_gw,copter.attitude_control->_desired_pitch_angle_rad,copter.attitude_control->_desired_pitch_angle_rate_rad);
}


Vector3f GCS_MAVLINK_Copter::get_state_JAI_3(){
    return Vector3f(copter.attitude_control->_beta_command,copter.attitude_control->_thr_out_command,0.0);//experiment use
    // return Vector3f(copter.attitude_control->_beta_command,copter.attitude_control->jai_thrust_out,0.0);//test
...
void GCS_MAVLINK_Copter::handleMessage(const mavlink_message_t &msg)
{
    ...
    switch (msg.msgid) {
    case MAVLINK_MSG_ID_JAI_OUT:
    {
        mavlink_jai_out_t packet;
        mavlink_msg_jai_out_decode(&msg, &packet);


        if(packet.rc_state==0){
            copter.rc_state_jai=packet.rc_state;
            copter.attitude_control->jai_thrust_out=0;
            copter.attitude_control->jai_thrust_state=packet.rc_state;
            break;

        }else if(packet.rc_state==1){
            copter.rc_state_jai=packet.rc_state;
            copter.jai_fake_desired_pitch=packet.pit_pwm;
            copter.jai_wheel_r=packet.wh_r;
            copter.jai_wheel_g=packet.wh_l;
            copter.attitude_control->jai_thrust_out=0;
            copter.attitude_control->jai_thrust_state=packet.rc_state;

            break;
        }else if(packet.rc_state==2){
            copter.attitude_control->jai_thrust_out=packet.u_thr;
            copter.attitude_control->jai_thrust_state=packet.rc_state;
            copter.rc_state_jai=packet.rc_state;
            copter.jai_fake_desired_pitch=packet.pit_pwm;
            copter.jai_wheel_r=packet.wh_r;
