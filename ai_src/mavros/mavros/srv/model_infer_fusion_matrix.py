...
def u_to_force(u):
    return thrust_value(throttle_value(u))
...
def accel(force,theta,result):

    # return result-4.5609*2*force*np.sin(theta)+24.03*np.sin(theta)
    return result.T-4.5609*2*force*np.sin(theta)+24.03*np.sin(theta)
...
class SegRos(object):
    ...
    def message_callback(self, msg):
        ...
        force_max=u_to_force(0.7)
        force_min=u_to_force(0.05)                # Check the condition
        if abs(msg.desired_dpit-msg.dpit)*105>max_accelerate:
            desired_accel=sign(msg.desired_dpit-result)*max_accelerate
            # Generate 1000 copies of the data array with varying 13th parameter
            variations = np.linspace(force_min, force_max,150)  # Generate variations from -0.02 to 0.02
            data_matrix = np.tile(data, (150, 1))  # Create 1000 copies of the original data
            data_matrix[:, -1] = variations  # Modify the last column with the variations
           
            data_tensor = torch.tensor(data_matrix, requires_grad=True, dtype=torch.float).to(self.device)
            predictions = self.predictor(data_tensor)
            predictions_np = predictions.detach().cpu().numpy()
            
            target=accel(variations,data_matrix[:,0],predictions_np)
            print(target.shape)
            differences = np.abs(desired_accel - target)
            print(differences.shape)
            min_index  = np.argmin(differences)
            closest_value = data_matrix[min_index]
            best_u = force_to_u(closest_value[-1])
            print("Success",closest_value[-1])
            self.new_traj.u_thr=best_u
