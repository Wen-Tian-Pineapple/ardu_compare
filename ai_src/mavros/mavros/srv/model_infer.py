...
class Net(nn.Module):
    def __init__(self, depth, D_in, D_h, D_out):
        ...
        if depth == 1:
            ...
            self.layers.append(nn.BatchNorm1d(D_h))  # BatchNorm after the first linear layer
            self.layers.append(nn.ReLU())

            for _ in range(1, depth - 1):
                self.layers.append(spectral_norm(nn.Linear(D_h, D_h)))
                self.layers.append(nn.BatchNorm1d(D_h))  # BatchNorm after each linear layer
                self.layers.append(nn.ReLU())

            self.layers.append(spectral_norm(nn.Linear(D_h, D_out)))
            self.layers.append(nn.BatchNorm1d(D_out))  # BatchNorm after the last linear layer
...
class SegRos(object):
    ...
    def message_callback(self, msg):
        ...
        sample_freq=105
        rate=1/(4.5609*2)
        if abs(msg.desired_dpit-result)<max_accelerate/sample_freq:
            desired_accel=(msg.desired_dpit-msg.dpit)*sample_freq
            while abs(desired_accel-result)>0.05:
                ulast = u
                u=rate*(desired_accel-result)
...
if __name__ == '__main__':
    ...
    model = Net(depth=depth, D_in=feature_num, D_h=512, D_out=output_dim).to(device)
    model_path = '/home/xxh/Doublebee/checkpoint/result_new2/split/5_layer/best_model_weights.pth'
